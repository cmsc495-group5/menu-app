...
class Demo extends Component {
    constructor(props) {
        ...
        this.mockItem = {
            id: 'test',
            name: 'Potato Skins',
            description: 'This is the description of potato skins that is customizable',
            internalDescription: 'This is a description that will not be used',
            image,
            price: 12.34,
            ordinal: 1,
            options: [],
        }
        this.items = [this.mockItem, {...this.mockItem, id: "test2", ordinal: 2}, {
            ...this.mockItem,
            id: "test3",
            ordinal: 3
        }]
    ...
    onChange = (e) => {
        console.log('onChangeCalled', e);
        const state = this.state
        state[e.target.name] = e.target.value;
        this.setState(state);
    }
    // we can use a service that can keep track of this and maintain its own state between sections
    ...
    render() {
        // we can sort items before mapping to ensure they go on the menu in order
        const items = this.items.map((item) =>
            <ItemCard data={item} itemUpdate={this.itemUpdate} key={item.id}/>
        )
        const demoHeight = "demoHeightLimit"
        return (
            <div>
                <Row>
                    <Col xs={12} md={4}>
